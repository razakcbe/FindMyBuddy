package com.findmybuddy.restws.utils;

public class FindMyBuddyConstants {

	public static final String TARGETPATH_FINDMYBUDDY = "findmybuddy";
	
	public static final String PATH_UPDATE_MY_LOCATION = "updatemylocation";
	
	public static final String PATH_CREATE_MY_DETAILS = "updatemydetail";
	
	public static final String PATH_GET_MY_DETAILS = "getmylocation/{mobileNo}";

	public static final String PATH_GET_MY_BUDDY_DETAILS = "getmybuddybdetail/{mobileNo}";
	
	public static final String PATH_GET_MY_IPDETAILS = "getmyIplocation/{ipAddress}";
}
